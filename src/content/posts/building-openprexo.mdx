---
title: "A local-first home for image generation"
description: A local-first image-generation workflow that keeps the CLI, desktop app, and provider jobs in step.
date: 2026-07-30
...
Underneath, the CLI and the desktop app use the same loopback-only API under `/v1`. A generation is submitted as a job with an idempotency key, and the job keeps its ID, attempt, state, provider and model, and produced media. The desktop app runs against an authenticated local sidecar rather than an open server. Remote providers are connected through BYOK, and the first-party OpenPrexo cloud backend is not operational.
